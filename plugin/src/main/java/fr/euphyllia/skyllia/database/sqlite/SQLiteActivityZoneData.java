...
public class SQLiteActivityZoneData extends ActivityZoneDataQuery {
    ...
    private static final String UPDATE_FLAGS = """
            UPDATE activity_zones SET allow_break = ?, allow_place = ?, allow_pvp = ?, allow_mob_attack = ?
            WHERE name = ?;
            """;
    ...
    public SQLiteActivityZoneData(DatabaseLoader loader) {
        this.loader = loader;
    }
    ...
    public boolean updateFlags(String name, boolean allowBreak, boolean allowPlace, boolean allowPvp, boolean allowMobAttack) {
        int affected = SQLExecute.update(loader, UPDATE_FLAGS, List.of(
                allowBreak ? 1 : 0, allowPlace ? 1 : 0, allowPvp ? 1 : 0, allowMobAttack ? 1 : 0, name
        ));
        return affected > 0;
    }
    ...
    private static ActivityZone map(ResultSet rs) throws SQLException {
        ...
        if (createdAtStr != null && !createdAtStr.isBlank()) {
            try {
                createdAt = Instant.parse(createdAtStr.replace(' ', 'T') + "Z");
        ...
        return new ActivityZone(
                ...
                rs.getInt("allow_break") != 0,
                rs.getInt("allow_place") != 0,
                rs.getInt("allow_pvp") != 0,
                rs.getInt("allow_mob_attack") != 0,
