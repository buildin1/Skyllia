...
public class WorldEditSchematicPaster {
    ...
    private static final Logger logger = LogManager.getLogger(WorldEditSchematicPaster.class);
    ...
    public CompletableFuture<Boolean> paste(Location loc, SchematicSetting settings) {
        ...
        loc.getWorld().getChunkAtAsync(loc.getBlockX() >> 4, loc.getBlockZ() >> 4, true, true).thenAccept(chunk -> {
            try {
