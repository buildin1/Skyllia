...
public class SQLiteUpgradeGenerator implements UpgradeGenerator {

    private static final Logger log = LoggerFactory.getLogger(SQLiteUpgradeGenerator.class);

    private static final String SELECT_LEVEL = """
            SELECT level FROM island_upgrade WHERE island_id = ?;
            """;

    private static final String UPSERT_LEVEL = """
            INSERT INTO island_upgrade (island_id, level)
            VALUES (?, ?)
            ON CONFLICT(island_id) DO UPDATE SET level = excluded.level;
            """;
    ...
    public SQLiteUpgradeGenerator(DatabaseLoader loader) {
        this.loader = loader;
    }
