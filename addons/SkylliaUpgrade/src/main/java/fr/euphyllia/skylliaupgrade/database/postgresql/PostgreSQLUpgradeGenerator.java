...
public class PostgreSQLUpgradeGenerator implements UpgradeGenerator {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLUpgradeGenerator.class);
    ...
    public PostgreSQLUpgradeGenerator(DatabaseLoader loader) {
        this.loader = loader;
    }

    @Override
    public UpgradeRecord getRecord(UUID islandId) {
        UpgradeRecord result = SQLExecute.queryMap(loader, SELECT_LEVEL, List.of(islandId), rs -> {
            try {
                if (rs.next()) {
    ...
    public Boolean setLevel(UUID islandId, int level) {
        int affected = SQLExecute.update(loader, UPSERT_LEVEL, List.of(islandId, level));
        return affected > 0;
    }
