...
public class UpgradeAdminCommand implements SubCommandInterface {
    ...
    /**
     * {@code /skylliadmin upgrade token <玩家> [数量]} —— 发放领地拓展令。
     * <p>
     * 令牌本来只能靠 SkylliaChallenge 的任务奖励发放（配置里填同款 customModelData），
     * 但那条路要求管理员手动配奖励，服务器没配就等于全服拿不到、岛屿升级卡死在这一环。
     * 这条命令直接调用 {@link UpgradeTokenItem#build(int)}，发出来的令牌带 PDC 标记，
     * 比挑战奖励那条路发的更防伪。
     * </p>
     */
    private void giveToken(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(Component.text("§c用法：/skylliadmin upgrade token <玩家> [数量]"));
            return;
        }
        ...
        if (args.length >= 3) {
            try {
                ...
                sender.sendMessage(Component.text("§c数量必须是整数，收到：§f" + args[2]));
                return;
            }
        ...
        target.getScheduler().run(SkylliaUpgrade.getInstance(), t -> {
            ItemStack token = UpgradeTokenItem.build(finalAmount);
            var overflow = target.getInventory().addItem(token);
            if (overflow.isEmpty()) {
                target.sendMessage(Component.text("§a你获得了 §d领地拓展令 x" + finalAmount + "§a。"));
                sender.sendMessage(Component.text("§a已发放 §d领地拓展令 x" + finalAmount + " §a给 §f" + target.getName()));
            } else {
                // 背包满了就掉在脚下，绝不能静默吞掉——那是玩家实打实的升级材料
                overflow.values().forEach(left -> target.getWorld().dropItemNaturally(target.getLocation(), left));
                target.sendMessage(Component.text("§a你获得了 §d领地拓展令 x" + finalAmount + "§a（背包已满，部分掉落在脚下）。"));
